...
public class BattleShip {
    ...
    public boolean placeShip(Ship ship, Orientation orientation, int x, int y) {
        ...
        if (orientation == Orientation.VERTICAL) {
            ...
            // Questo blocco serve a registrare nella nave le coordinate che occupa sulla griglia.
            for (int i = 0; i < dim; i++) {
                table[x + i][y] = Moves.SHIP;
                rows[i] = x + i; //Poi queste coordinate vengono salvate dentro l'oggetto ship tramite i setter.
                col[i] = y;
            }
            // Questo serve dopo, nel metodo isSunk, per sapere quali celle controllare per vedere se la nave è affondata
    ...
    public boolean isSunk(Ship ship) {
        if (ship.getCoordinationX() == null) return false;
